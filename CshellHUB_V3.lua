...
-- [[ TWEEN TP ]] --
function TP(targetPos)
    local Character = LocalPlayer.Character
    if Character and Character:FindFirstChild("HumanoidRootPart") then
        local distance = (targetPos.Position - Character.HumanoidRootPart.Position).Magnitude
        local speed = 300
        if distance < 300 then speed = 600 end
        local tween = TweenService:Create(Character.HumanoidRootPart, TweenInfo.new(distance/speed, Enum.EasingStyle.Linear), {CFrame = targetPos})
        tween:Play()
        return tween
    end
end
        ...
        if _G.AutoFarm then
            pcall(function()
                ...
                if not LocalPlayer.PlayerGui.Main.Quest.Visible then
                    -- Nhận Quest
                    TP(q.QPos)
                    if (q.QPos.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 10 then
...
local function LoadMainHub()
    ...
    Title.Parent = MainFrame; Title.Text = "CSHELL HUB v3 FULL"; Title.Font = Enum.Font.GothamBold; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 28; Title.Size = UDim2.new(1, 0, 0, 60); Title.BackgroundTransparency = 1
...
local function LoginUI()
    ...
    T.Parent = Main; T.Text = "CSHELL LOGIN v3"; T.Font = Enum.Font.GothamBold; T.TextColor3 = Color3.fromRGB(255, 255, 255); T.TextSize = 35; T.Size = UDim2.new(1, 0, 0, 80); T.BackgroundTransparency = 1
