-- [[ CSHELL HUB - PREMIUM FINAL SECURE V4 ]] --
...
Status.Parent = MainLogin; Status.Text = "Cshell HUB Final Update"; Status.TextColor3 = Color3.fromRGB(180, 180, 180); Status.Position = UDim2.new(0, 0, 0.88, 0); Status.Size = UDim2.new(1, 0, 0, 25); Status.BackgroundTransparency = 1; Status.Font = Enum.Font.Gotham; Status.TextSize = 14
...
local function Verify(key)
    ...
    if res and res.success then
        ...
        local success, result = pcall(function()
            ...
            -- REBRANDING SIÊU TỐC NGAY TRONG LOADER
            source = source:gsub("Binh Hub Kaitun", "CSHELL HUB PREMIUM")
            ...
            -- PHÓNG TO GUI
            source = source:gsub("Size = UDim2.new%(0, 1033, 0, 583%)", "Size = UDim2.new(0, 1200, 0, 700)")
            source = source:gsub("Position = UDim2.new%(0.499266863, %-516, 0.498697907, %-291%)", "Position = UDim2.new(0.5, -600, 0.5, -350)")
            -- FIX LỖI NIL VALUE
            source = source:gsub("settings%(%)", "pcall(function() return settings() end)")
        ...
        if success then CshellLogin:Destroy() else
            Status.Text = "Error: " .. tostring(result):sub(1, 40); Status.TextColor3 = Color3.fromRGB(255, 50, 50)
        end
    else
